...
void getGyrometer(int16_t *gyrx,int16_t *gyry, int16_t* gyrz)
{
	uint8_t addressWrite=0xd4;
	uint8_t addressRead=0xd5;
...
void init_gyroscope(){
    ...
    // 208 Hz ODR (0b0101), ±500 dps
    configData[0] = 0x11;                // CTRL2_G 暫存器位址
    configData[1] = 0x12;

    // 一次性寫入暫存器位址及設定值
    HAL_I2C_Master_Transmit(&hi2c2, LSM6DSL_ADDR, configData, 2, 100);
}
...
void init_SMD(void)
{
	...
	cmd[0] = 0x01;
	cmd[1] = 0x80;
	HAL_I2C_Master_Transmit(&hi2c2,addressWrite,cmd,2,100);
...
extern uint8_t SMD_FLAG;  //聲明外部變數
