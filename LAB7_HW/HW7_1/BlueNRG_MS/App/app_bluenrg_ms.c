...
void MX_BlueNRG_MS_Init(void)
{
  ...
  const char *name = "BlueNRG";//修改名字
//  const char *name = "JimmyStm32";//修改名字
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    PRINTF("Error adding Control Service\r\n");
  }


  ret = Add_MVACC_Service();
  if (ret != BLE_STATUS_SUCCESS) {
	  PRINTF("Error adding Control Service\r\n");
  }
...
#if !USE_BUTTON
      HAL_Delay(BLE_Freq); //此修改BLE的傳送速率
#endif
    }
#if USE_BUTTON
