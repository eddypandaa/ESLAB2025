 ...
 * @retval None
 */
extern uint16_t ControlCharHandle;
extern uint16_t ble_update_interval;
uint16_t BLE_Freq = 100;

// 建議用 bluenrg_events.h 定義內建的 attr_modified struct
  ...
  case EVT_VENDOR:
    {
      ...
      case EVT_BLUE_GATT_ATTRIBUTE_MODIFIED:
		  {
			  evt_gatt_attr_modified_IDB05A1 *am = (void*)blue_evt->data;

			  if (am->attr_handle == ControlCharHandle + 1)
			  {
				  if (am->data_length >= 2)
				  {
//					  BLE_Freq = am->att_data[0] | (am->att_data[1] << 8);

					  char hex_str[5];
					  sprintf(hex_str, "%02X%02X", am->att_data[0], am->att_data[1]);
					  BLE_Freq = atoi(hex_str);
